"""Behavior of the recall route through the single dispatch path (Phase A).

recall is now the registered memory.recall tool, executed via the shared
ToolExecutor and rendered by tool_dispatch. These tests still drive the public
assistant.dispatch entry point, but patch the relocated collaborators:
memory_store (the tool's data source) and tool_dispatch.ask_local (the renderer's
summarization step). The behavioral contract is unchanged from Phase 1.
"""
